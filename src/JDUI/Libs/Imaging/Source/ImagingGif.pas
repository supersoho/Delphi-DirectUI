...
begin
  ...
  FDelayTimeLists.Clear;
  //FDelayTime:=0;//默认为0就是100毫秒
  AddMasks(SGIFMasks);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //这里还不对 取得是最后的值
        if GraphicExt.DelayTime = 0 then GraphicExt.DelayTime := 10;
        FDelayTimeLists.Add(inttostr(GraphicExt.DelayTime));
        //FDelayTime:=GraphicExt.DelayTime;
      end
      else
...
begin
  ...
  begin
    ...
    begin
      ...
      CachedIndex := -1;
      if not OnlyFirstLevel then
       { try
         //AnimFrames[Idx].Extra.Delay := GraphicExt.DelayTime;
        except
        end;  }
