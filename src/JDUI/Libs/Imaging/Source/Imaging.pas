...
type
  ...
  protected
    ...
    FDelayTimeLists:TStringList;//主要为了针对gif
    //FdelayTime:Word;//主要为了针对gif
  ...
  public
    ...
    Property DelayTimeLists:Tstringlist Read FDelayTimeLists;

    //Property DelayTime:Word Read FdelayTime;
  end;
  {$TYPEINFO OFF}
...
function LoadMultiImageFromFileEx(const FileName: string;
  var Images: TDynImageDataArray):TImageFileFormat;
...
function LoadMultiImageFromFileEx(const FileName: string; var Images:
  TDynImageDataArray): TImageFileFormat;
{var
  Format: TImageFileFormat; }
begin
  Assert(FileName <> '');
  Result := nil;
  Result := FindImageFileFormatByExt(DetermineFileFormat(FileName));
  if Result <> nil then
  begin
    FreeImagesInArray(Images);
    Result.LoadFromFile(FileName, Images);
  end;
end;
...
begin
  ...
  begin
    Result := TGIFFileFormat.Create;
    exit;
  end;

//下面的处理方法明显是全局处理线程是不安全的
  Result := nil;
...
begin
  ...
  FDelayTimeLists:=TStringList.Create;
end;
