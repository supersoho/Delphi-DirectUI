...
type
  ...
  public
    ...
    Function LoadMultiFromFileEx(const FileName: string):TImageFileFormat;
...
begin
  ...
  begin
    ...
    if Imaging.TestImage(TSingleImage(Source).FImageData) then
    begin
      Imaging.CloneImage(TSingleImage(Source).FImageData, FImageData);
      DoDataSizeChanged;
    end
    else
    begin
  ...
  begin
    ...
    begin
      ...
      if Imaging.TestImage(TMultiImage(Source).FPData^) then
      begin
        Imaging.CloneImage(TMultiImage(Source).FPData^, FImageData);
        DoDataSizeChanged;
      end
      else
      begin
...
begin
  ...
  begin
    Imaging.FreeImagesInArray(FDataArray);
    SetLength(FDataArray, Length(TMultiImage(Source).FDataArray));
    for I := 0 to GetImageCount - 1 do
    begin
      // Clone only valid images
      if Imaging.TestImage(TMultiImage(Source).FDataArray[I]) then
        Imaging.CloneImage(TMultiImage(Source).FDataArray[I], FDataArray[I])
      else
        Imaging.NewImage(DefaultWidth, DefaultHeight, ifDefault, FDataArray[I]);
    end;
    SetActiveImage(0);
  ...
  begin
    ...
    Imaging.FreeImagesInArray(FDataArray);
    SetLength(FDataArray, Length(Arr));
    for I := 0 to GetImageCount - 1 do
    begin
      // Clone only valid images
      if Imaging.TestImage(Arr[I]) then
        Imaging.CloneImage(Arr[I], FDataArray[I])
      else
        Imaging.NewImage(DefaultWidth, DefaultHeight, ifDefault, FDataArray[I]);
    end;
    SetActiveImage(0);
...
Function TMultiImage.LoadMultiFromFileEx(const FileName: string):TImageFileFormat;
begin
  Result := Imaging.LoadMultiImageFromFileEx(FileName, FDataArray);
  if Result <> nil then
  SetActiveImage(0);
end;
