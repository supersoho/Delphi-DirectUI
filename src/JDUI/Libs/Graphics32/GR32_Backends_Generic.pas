...
begin
  ...
  try
    WinApi.Windows.GetTempPath(MAX_PATH, PC);
    Result := TFileName(PC);
  finally
    StrDispose(PC);
  end;
...
{$ELSE}
  ...
  begin
    PC := StrAlloc(MAX_PATH + 1);
    WinApi.Windows.GetTempFileName(PChar(GetTempPath), PChar(Prefix), 0, PC);
    Result := string(PC);
    StrDispose(PC);
  end;
...
begin
  ...
  // create map
  MapHandle := WinApi.Windows.CreateFileMapping(MapFileHandle, nil, PAGE_READWRITE, 0, NewWidth * NewHeight * 4, nil);
