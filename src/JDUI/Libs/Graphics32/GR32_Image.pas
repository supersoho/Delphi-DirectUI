...
{$ENDIF}
  ...
  public
    ...
    property PaintToCanvas: Boolean read FPaintToCanvas write FPaintToCanvas;
...
{$ENDIF}
  ...
  TPaintLayerHandler = procedure(Dest: TBitmap32; Layer: TCustomLayer) of object;
  ...
  private
    ...
    FOnPaintLayer: TPaintLayerHandler;
  ...
  public
    ...
    property OnPaintLayer: TPaintLayerHandler read FOnPaintLayer write FOnPaintLayer;
  end;
...
begin
  if not FPaintToCanvas then Exit;
...
begin
  inherited;
  if (Parent = nil) or (csDesigning in ComponentState) then ResizeBuffer;
  FBufferValid := False;
end;
...
procedure TCustomImage32.GetBuffer;
begin
  if PaintToCanvas then
    Invalidate
  else
    Paint;
end;
...
begin
  ...
  for I := 0 to Layers.Count - 1 do
    ...
    begin
      if (Assigned(FOnPaintLayer)) then FOnPaintLayer(Dest, Layers.Items[I]);
