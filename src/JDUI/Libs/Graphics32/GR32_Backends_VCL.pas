...
uses
  SysUtils, Classes, WinApi.Windows, VCL.Graphics, GR32, GR32_Backends, GR32_Containers,
...
begin
  ...
  begin
    ...
    try
      DC := StockBitmap.Canvas.Handle;
      OldFont := SelectObject(DC, Font.Handle);
      WinApi.Windows.GetTextExtentPoint32W(DC, PWideChar(Text), Length(Text), Result);
      SelectObject(DC, OldFont);
    finally
      StockBitmap.Canvas.Unlock;
    end;
...
begin
  if (FHDC = 0) and (FBitmapHandle <> 0) then
  begin
    try
      FHDC := CreateCompatibleDC(0);
      if FHDC = 0 then
...
begin
  ...
  begin
    ...
    if FHDC <> 0 then SelectObject(FHDC, StockFont);
