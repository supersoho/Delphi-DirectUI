...
procedure AlphaBltSSE(dst, src: PAnsiChar; w, h: Integer);
var
  wmul4: Integer;
begin
	wmul4 := w shl 2;
...
procedure MMXAlphaBlend(
	   lpDst: PDWORD;        // 目标缓冲
     iDstX: DWORD;        // 目标位置
     iDstY: DWORD;        // 目标位置
     iDstPitch: DWORD;    // 目标缓冲的pitch
     lpSrc: PDWORD;        // 原色彩缓冲
     iSrcX: DWORD;        // 原色彩位置
     iSrcY: DWORD;        // 原色彩位置
     iSrcW: DWORD;        // 原缓冲的尺寸
     iSrcH: DWORD;        // 原缓冲的尺寸
     iSrcPitch: DWORD    // 原色彩pitch
     );
var
  lpLinearDstBp: PDWORD;
  lpLinearSrcBp: PDWORD;
begin
     // 需要是4的倍数
     lpLinearDstBp := lpDst;
     lpLinearSrcBp := lpSrc;
     Inc(lpLinearDstBp, (iDstX) + (iDstY * iDstPitch div sizeof(DWORD))); //base pointer for linear destination
     Inc(lpLinearSrcBp, (iSrcX) + (iSrcY * iSrcPitch div sizeof(DWORD))); //base pointer for linear source
     ...
     asm
			...
			mov edx,$FFFFFFFF //255-Alpha mask
		@@MainLoop:
            pxor mm7,mm7
            movd mm5,eax
            movd mm6,edx
		...
		@@BeginPixel:
            ...
            sub ebx,1;                                 // 宽度减4
            test ebx,$FFFFFFFF;                       // check if only 0 pixels left
...
var
  ...
  iSrcPitch: DWORD;
begin
  lpDst := PDWORD(Dst.Bits);
  iDstPitch := Dst.Width * 4;

  lpSrc := PDWORD(Src.Bits);
  iSrcX := SrcX;
  iSrcY := SrcY;
  iSrcW := Src.Width;
  iSrcH := Src.Height;
  iSrcPitch := Src.Width * 4;

  if DstRect.Left > Dst.Width then Exit;
  if DstRect.Top > Dst.Height then Exit;

  if DstRect.Left < 0 then
  begin
    iSrcX := iSrcX + Abs(DstRect.Left);
    iSrcW := iSrcW - Abs(DstRect.Left);
  ...
  begin
    iSrcY := iSrcY + Abs(DstRect.Top);
    iSrcH := iSrcH - Abs(DstRect.Top);

    DstRect.Top := 0;
  end;

  if DstRect.Right > Dst.Width then
  begin
    iSrcW := iSrcW - (DstRect.Right - Dst.Width);
    DstRect.Right := Dst.Width;
    if iSrcW <= 0 then Exit;
  end;

  if DstRect.Bottom > Dst.Height then
  begin
    iSrcH := iSrcH - (DstRect.Bottom - Dst.Height);
    DstRect.Bottom := Dst.Height;
    if iSrcH <= 0 then Exit;
  end;

  if DstRect.Width <= 0 then Exit;
  if DstRect.Height <= 0 then Exit;

  iDstX := DstRect.Left;
  iDstY := DstRect.Top;
  if iSrcH > DstRect.Height then iSrcH := DstRect.Height;
  if iSrcW > DstRect.Width then iSrcW := DstRect.Width;

  try
    //Inc(lpDst, (Dst.Height - iDstY) * iDstPitch + iDstX * 4);
    //Inc(lpSrc, (Src.Height - iSrcY) * iSrcPitch + iSrcX * 4);
    //AlphaBltSSE(PAnsiChar(lpDst), PAnsiChar(lpSrc), iSrcW, iSrcH);
    ...
    //On E: Exception do OutputDebugString(PChar('MMXAlphaBlend: ' + E.Message));
  end;
end;
...
begin
  ...
  case CombineOp of
    ...
    dmTransparent:
      ...
      begin
        MMXBlendBlock(Dst, DstRect, Src, SrcX, SrcY);
        {
        for DstY := DstRect.Top to DstRect.Bottom - 1 do
        begin
          SP := SrcP;
