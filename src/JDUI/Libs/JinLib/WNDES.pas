...
uses
  Windows, Messages, ShlObj, WinSock2, IniFiles, ActiveX, SysUtils,
  Classes;
...
var
  ...
  TempResult: UTF8String;
  I: Integer;
  FBuffer: Array of Byte;
begin
  //TempResult := UTF8Encode(EncryStr(Str, Key));
  TempResult := UTF8Encode(Str);

  SetLength(FBuffer, Length(TempResult));
  CopyMemory(@FBuffer[0], PAnsiChar(TempResult), Length(TempResult));

  StrResult := '';
  for I := 0 to Length(TempResult) - 1 do
  begin
    Temp := Format('%x', [FBuffer[I]]);
...
var
  ...
  FBuffer: Array of Byte;
begin
  SetLength(FBuffer, Length(StrHex) div 2);
  Str := '';
  for I := 0 to Length(StrHex) div 2 - 1 do
  begin
    Temp := Copy(StrHex, I * 2 + 1, 2);
    FBuffer[I] := HexToInt(Temp);
  end;
  SetLength(TempResult, Length(StrHex) div 2);
  CopyMemory(PAnsiChar(TempResult), @FBuffer[0], Length(TempResult));
  Str := UTF8ToUnicodeString(TempResult);
