...
function MD5En(M: string): string;
function MD5DigestToStr(M: MD5Digest): string;
function InternalMD5Stream(Stream: TStream; const BufSize: Cardinal; var D: MD5Digest): Boolean;
...
begin
  InternalMD5Stream(Stream, 4096 * 1024, Result);
end;
...
function MD5En(M: string): string;
begin
  Result := MD5Print(MD5String(M)).ToLower;
end;
...
begin
	MD5Init(Context);
	MD5Update(Context, PAnsiChar(UTF8Encode(M)), length(UTF8Encode(M)));
...
begin
  ...
  try
    dwBytesInBlock := GetFileSize(FileHandle, nil);
		MapHandle := CreateFileMapping(FileHandle, nil, PAGE_READONLY, 0, dwBytesInBlock, nil);
		if MapHandle <> 0 then try
			ViewPointer := MapViewOfFile(MapHandle, FILE_MAP_READ, 0, 0, dwBytesInBlock);
			if ViewPointer <> nil then
      try
				MD5Update(Context, ViewPointer, dwBytesInBlock);
			finally
