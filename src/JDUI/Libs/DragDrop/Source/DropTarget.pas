...
uses
  DragDrop,
  WinApi.Windows, WinApi.ActiveX, Classes, Vcl.Controls, WinApi.CommCtrl, Vcl.ExtCtrls, Vcl.Forms;
...
{$endif}
  ...
  published
    ...
    property DontGetData: Boolean read FDontGetData write FDontGetData default False;
...
begin
  ...
  try
    ...
    begin
      ...
      // get data.
      try
        if (FDontGetData or FGetDataOnEnter or GetData(dwEffect)) then
        begin
          //if ((not FDontGetData) and (not FGetDataOnEnter)) then GetData(dwEffect);
          if (not AsyncTransfer) then
